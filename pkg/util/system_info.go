...
func GetSystemCPUInfo() (SystemCPUInfo, error) {
	...
	// Go through each CPU core and count the number of different CoreIds
	var coreCount int32
	coreIds := make(map[string]bool)
	for _, core := range cpuInfo {
		if _, ok := coreIds[core.CoreID]; !ok {
			coreIds[core.CoreID] = true
			coreCount++
		}
	}
	...
	cpu := SystemCPUInfo{
		...
		CoresCount:   int32(coreCount),
		ThreadsCount: int32(len(cpuInfo)),
	}
